...
class PostgresHandlerAgent:
    ...
    def __init__(self, 
                db_url: str = "postgresql://postgres:root@localhost:5432/parseqri_db", 
                schema: str = "public"):
        """
    ...
    def create_and_populate_table(self, user_id: str, csv_path: str, 
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                self.logger.info(
                    "to_sql completed: schema=%s table=%s inserted_rows=%s",
                    self.schema,
                    qualified_table_name,
                    len(df)
                )
                verification_query = text(
                    f'SELECT COUNT(*) AS row_count FROM "{self.schema}"."{qualified_table_name}"'
                )
                row_count = conn.execute(verification_query).scalar()
                self.logger.info(
                    "Upload verification: schema=%s table=%s stored_rows=%s",
                    self.schema,
                    qualified_table_name,
                    row_count
                )
            ...
            self.logger.exception("Error creating/populating table: %s", e)
            return False, str(e), ""
