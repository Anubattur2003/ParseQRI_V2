...
class RedisCacheAgent:
    ...
    def _connect(self, host: str, port: int, db: int):
        ...
        try:
            ...
            # Keep logs ASCII-only to avoid Windows console UnicodeEncodeError.
            print(f"[OK] Redis connected at {host}:{port}")
        except Exception as e:
            self.redis_available = False
            print(f"[WARN] Redis unavailable: {str(e)}")
    ...
    def process(self, context: QueryContext) -> AgentResponse:
        ...
        try:
            ...
            if cached_data:
                ...
                print(f"[HIT] Cache HIT for: {context.user_question[:50]}...")
                ...
                print(f"[MISS] Cache MISS for: {context.user_question[:50]}...")
                return AgentResponse(
                    success=True,
                    message="Cache miss",
                    data={"cache_hit": False}
            ...
            print(f"[WARN] Cache check error: {str(e)}")
            return AgentResponse(
                success=True,
    ...
    def cache_query(self, context: QueryContext):
        ...
        try:
            ...
            print(f"[INFO] Cached: {context.user_question[:50]}... (TTL: {self.ttl_seconds}s)")
            
        except Exception as e:
            print(f"[WARN] Cache save error: {str(e)}")
    ...
    def clear_cache(self):
        ...
        try:
            ...
            if keys:
                ...
                print(f"[INFO] Cleared {len(keys)} cache entries")
        except Exception as e:
            print(f"[WARN] Cache clear error: {str(e)}")
