...
class MetadataIndexerAgent:
    ...
    def _get_user_collection(self, user_id):
        ...
        if user_id not in self.chroma_clients:
            try:
                ...
                # Disable telemetry to avoid posthog `capture()` signature issues.
                from chromadb.config import Settings as ChromaSettings
                self.chroma_clients[user_id] = chromadb.PersistentClient(
                    path=user_dir,
                    settings=ChromaSettings(
                        anonymized_telemetry=False,
                        # Fully disable posthog by swapping the telemetry impl.
    ...
    def process(self, context: QueryContext) -> AgentResponse:
        ...
        try:
            ...
            if hasattr(context, 'csv_file') and context.csv_file:
                ...
                # Ensure we store only the base table name (without user_id prefix)
                raw_table_name = metadata.get("table_name", context.table_name)
                # Strip user_id prefix if present to avoid double-prefixing later
                if raw_table_name and raw_table_name.startswith(f"{context.user_id}_"):
                    base_table_name = raw_table_name[len(f"{context.user_id}_"):]
                else:
                    base_table_name = raw_table_name
    ...
    def _parse_llm_json(self, content: str) -> Dict[str, Any]:
        """
        Parse JSON produced by the LLM, even if it includes code fences or extra text.
        """
        ...
        try:
            ...
            # Some models return JSON-like dicts; fall back to Python literal parsing.
            import ast
            parsed = ast.literal_eval(json_str)
            if not isinstance(parsed, dict):
                raise ValueError("Parsed JSON was not a dictionary")
            return parsed
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # Parse JSON response
                    metadata = self._parse_llm_json(content)
    ...
    def save_metadata_to_chroma(self, user_id: str, table_name: str, 
        ...
        # Create document ID from user_id and table_name (using prefix pattern)
    ...
    def search_relevant_metadata(self, user_id: str, query_text: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Construct full table name with user_id PREFIX (matching postgres_handler convention)
            # BUT check if it already has the prefix to avoid double-prefixing
            if base_table_name:
                if base_table_name.startswith(f"{user_id}_"):
                    # Already has prefix, use as-is
                    full_table_name = base_table_name
                else:
                    # Add the prefix
                    full_table_name = f"{user_id}_{base_table_name}"
            else:
                full_table_name = None
            
            return {
                "document_id": document_id,
                "table_name": full_table_name,  # Return full table name with user_id prefix
