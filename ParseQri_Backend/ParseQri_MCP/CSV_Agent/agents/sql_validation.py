...
class SQLValidationAgent:
    ...
    def process(self, context: QueryContext) -> AgentResponse:
        ...
        try:
            ...
            if context.table_name and context.user_id:
                ...
                # Check if it already has user_id as prefix or suffix
                if context.table_name.startswith(f"{context.user_id}_"):
                    # Already has prefix - use as is
                    postgres_table = context.table_name
                    print(f"Table already has user_id prefix, using: {postgres_table}")
                elif context.table_name.endswith(f"_{context.user_id}"):
                    # Has suffix - use as is
                    postgres_table = context.table_name
                    print(f"Table already has user_id suffix, using: {postgres_table}")
                else:
                    # Add prefix (standard pattern)
                    postgres_table = f"{context.user_id}_{context.table_name}"
            ...
            # Ensure table names remain aligned and quoted after LLM "fixes".
            validated_query = validation_result.get("sql_query", sanitized_query)
            validated_query = self._normalize_table_references(
                validated_query,
                context.table_name,
                context.user_id
            )
            validation_result["sql_query"] = validated_query
            self.logger.info("Validated SQL query: %s", validated_query)
    ...
    def fallback_fix_query(self, sql_query: str) -> str:
        ...
        if "SELECT" not in query.upper() and "FROM" not in query.upper():
            ...
            if match:
                ...
                # Quote table identifier to support names like `1_loan_data`
                raw_table = table.strip()
                if raw_table.startswith('"') and raw_table.endswith('"'):
                    quoted_table = raw_table
                elif "." in raw_table:
                    schema_part, table_part = raw_table.split(".", 1)
                    quoted_table = f'"{schema_part}"."{table_part}"'
                else:
                    quoted_table = f'"{raw_table}"'

                return f"SELECT COUNT(*) AS {alias} FROM {quoted_table} WHERE {where};"
    ...
    def _normalize_table_references(self, sql_query: str, table_name: str, user_id: str) -> str:
        """
        Normalize FROM/JOIN table references:
        - replace stale user table names with context.table_name
        - ensure table names that start with digits are double-quoted
        """
        if not sql_query:
            return sql_query
        query = sql_query
        if table_name:
            # Rewrite simple stale references like 4_movies -> 4_movies_data when context knows true table.
            stale_pattern = re.compile(r'(?i)\b(?:from|join)\s+"?([a-zA-Z0-9_]+)"?')
            for match in stale_pattern.finditer(query):
                found_table = match.group(1)
                if user_id and found_table.startswith(f"{user_id}_") and found_table != table_name:
                    query = re.sub(
                        rf'(?i)\b{re.escape(found_table)}\b',
                        table_name,
                        query
                    )
            
            # Quote FROM/JOIN table references to safely support names like 4_movies_data.
            def quote_from_join(m: re.Match) -> str:
                keyword = m.group(1)
                table = m.group(2)
            ...
            query = re.sub(r'(?i)\b(from|join)\s+("?[\w]+"?)', quote_from_join, query)
        return query