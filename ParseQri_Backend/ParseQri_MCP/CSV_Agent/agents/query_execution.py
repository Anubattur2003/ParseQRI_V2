...
class QueryExecutionAgent:
    ...
    def __init__(self, postgres_url="postgresql://postgres:root@localhost:5432/parseqri"):
        """
    ...
    def process(self, context: QueryContext) -> AgentResponse:
        ...
        try:
            ...
            self.logger.info(
                "Executing SQL for user=%s table=%s query=%s",
                context.user_id,
                context.table_name,
                sql_query
            )
    ...
    def execute_postgres_query(self, query: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            query = self._quote_problematic_table_identifiers(query)
            self.logger.info("Final SQL query to execute: %s", query)
            ...
            self.logger.exception("Error executing PostgreSQL query: %s", e)
            self.logger.error("Query was: %s", query)
            return None

    def _quote_problematic_table_identifiers(self, query: str) -> str:
        """
        Ensure FROM/JOIN table names are quoted when unquoted identifiers are unsafe.
        Example: FROM 4_movies_data -> FROM "4_movies_data"
        """
        def repl(m: re.Match) -> str:
            ...
            # Keep schema-qualified names valid.
            if "." in table:
                schema, tbl = table.split(".", 1)
                if tbl.startswith('"') and tbl.endswith('"'):
                    return f'{keyword} {schema}.{tbl}'
                return f'{keyword} {schema}."{tbl}"'
            # Quote if starts with digit or contains non-word chars.
            if table and (table[0].isdigit() or not re.match(r'^\w+$', table)):
                return f'{keyword} "{table}"'
            return f"{keyword} {table}"
        
        return re.sub(r'(?i)\b(from|join)\s+("?[\w\.]+"?)', repl, query)
