...
class MSSQLConnector:
        ...
        if schema_name:
            query = """
                ...
                ORDER BY 
                    ORDINAL_POSITION;
            """
            params = {"table_name": actual_table_name, "schema_name": schema_name, "database": self.config.database_name}
        else:
            query = """
                SELECT 
                    COLUMN_NAME as column_name,
                ...
                ORDER BY 
                    ORDINAL_POSITION;
            """
            params = {"table_name": actual_table_name, "database": self.config.database_name}
    ...
    def list_tables(self) -> List[str]:
        """Lists all tables in the current database with schema prefixes."""
        try:
            query = """
                SELECT 
            ...
            # Return schema-qualified table names (schema.table)
